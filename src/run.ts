...
import * as availableCommands from "./model/commands.ts";
...
const run = (commandStrings: Array<string>): Promise<any> => {
  ...
  return Promise.all(
    commandStrings
      .map((s) => availableCommands[s])
      .map(queue.enqueue),
  );
};
export default run;

const usageAndExit = (code: number = 1) => {
  console.error(`
Usage:   ./run.ts <command...>

         Available commands:
${Object.keys(availableCommands).map((command) => `            ${command}`)}
...
if (import.meta.main) {
  ...
  await run(Deno.args).then(() => {}, console.error.bind(console));
}
