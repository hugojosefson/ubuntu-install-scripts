...
import { availableCommands, CommandName } from "./commands/index.ts";
import { Command } from "./model/command.ts";
import { Queue } from "./model/queue.ts";

const run = (commandStrings: Array<string>): Promise<any> => {
  const queue = new Queue();
  const commandNames = commandStrings.map((s) => s as CommandName);
  commandNames.filter((s) => !availableCommands[s]).forEach((s) =>
    usageAndExit(2, `"${s}" is not a valid command.`)
  );
  return Promise.all(
    commandNames
      .map((s) => availableCommands[s] as Command)
